...
async def createPackage(pkg_name: str, pkg_version: str, pkg_dependencies: list, pkg_maintainer: str, pkg_update_key: str, pkg_server_path: str, pkg_hash: str, pkg_size: int):
    ...
    check_name_existence = await conn.fetch("select * from packages_list where name = $1", pkg_name)
    check_server_path_existence = await conn.fetch("select * from packages_list where path = $1", pkg_server_path)
